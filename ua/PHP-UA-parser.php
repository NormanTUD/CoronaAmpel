...
if(preg_match('/OpenVAS/', $UA)) {
	$infos['browser_name'] = 'OpenVAS';
	$infos['platfrm_name'] = 'OpenVAS';
} elseif(preg_match('/facebookexternalhit/', $UA)) {
	$infos['browser_name'] = 'Facebook Crawler';
	$infos['platfrm_name'] = 'Facebook Crawler';
} elseif(preg_match('/Googlebot/', $UA)) {
	$infos['browser_name'] = 'Google Crawler';
	$infos['platfrm_name'] = 'Google Crawler';
} elseif(preg_match('/SemrushBot/', $UA)) {
	$infos['browser_name'] = 'SemrushBot';
	$infos['platfrm_name'] = 'SemrushBot';
}

// Mozilla compatible ?
elseif (preg_match('#^Mozilla/([0-9]{1,}\.[0-9]{1,}) #i', $UA, $matches)) {
		...
		if (stripos($m_oses[1], 'Win') !== FALSE) {
			...
			if (preg_match('#NT (([0-9]{1,}\.?){0,})#i', $UA_Brw, $m_win_version)) {
				switch (substr($m_win_version[1], 0, 3) ) {
					...
					case '5.1': $infos['platfrm_vers'] = 'XP'; break;
					case '5.2': $infos['platfrm_vers'] = 'Server 2003'; break;
					...
					case '10.0': $infos['platfrm_vers'] = '10.0'; break;
